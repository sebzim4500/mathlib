...
begin
  ...
  { assume eq,
    by_cases a = b,
    { subst h, simp only * with multiset at * },
...
@[simp,multiset] theorem card_singleton (a : α) : card (a::0) = 1 := by simp
...
⟨λ h, (eq_of_le_of_card_le (zero_le _) (le_of_eq h)).symm, λ e, by simp only [e] with multiset⟩
...
@[simp,multiset] theorem mem_singleton {a b : α} : b ∈ a::0 ↔ b = a := by simp
...
@[simp,multiset] lemma repeat_succ (a : α) (n) : repeat a (n+1) = a :: repeat a n := by simp only [repeat] with multiset

@[simp,multiset] lemma repeat_one (a : α) : repeat a 1 = a :: 0 := by simp
...
begin subst h, simp only with multiset at hf, simp only [map_congr hf] end
...
foldr (*) (λ x y z, by simp only [mul_left_comm]) 1
...
multiset.induction_on m (by simp with multiset) (by simp with multiset)
...
multiset.induction_on m (by simp with multiset) (assume a m ih, by simp [ih]; cc)
...
multiset.induction_on m (by simp with multiset) (assume a m ih, by simp [ih])
...
multiset.induction_on s (by simp with multiset) (assume a s ih, by simp [ih, mul_add] with multiset)

lemma sum_map_mul_right [semiring β] {b : β} {s : multiset α} {f : α → β} :
  sum (s.map (λa, f a * b)) = sum (s.map f) * b :=
multiset.induction_on s (by simp with multiset) (assume a s ih, by simp [ih, add_mul] with multiset)
...
multiset.induction_on S (by simp with multiset) (by simp with multiset)
...
by simp [bind] with multiset {contextual := tt}
...
multiset.induction_on m (by simp only with multiset) (by simp only with multiset {contextual := tt})
...
multiset.induction_on s (by simp only with multiset) (assume a s ih, by simp [ih, cons_bind] with multiset)
...
by simp only [product] with multiset
...
by simp [product, repeat, (∘), mul_comm] with multiset
...
| ⟨a, b⟩ := by simp only [multiset.sigma, and_assoc, and.left_comm] with multiset
...
begin
  revert t,
  refine multiset.induction_on s (by simp only with multiset) (λ a s IH t h, _),
...
begin
  rw [inter_comm],
  revert s, refine multiset.induction_on t (by simp only with multiset) (λ a t IH s, _),
  by_cases a ∈ s,
...
begin
  ...
  { simp with multiset },
  { assume a s ih, simp [ih, add_mul, mul_comm, mul_left_comm, mul_assoc, sum_map_mul_left.symm] with multiset },
end
...
by induction n; simp only [*, succ_smul', succ_mul] with multiset
...
multiset.induction_on s (by simp only with multiset) (by simp only [rel_cons_left] with multiset {contextual := tt})
...
multiset.induction_on s₂
  ...
  (by simp only with multiset {contextual := tt}; cc)
...
multiset.induction_on s (by simp only with multiset) (by simp only with multiset {contextual := tt}; cc)
...
multiset.induction_on s (by simp only with multiset) (by simp only [hm] with multiset {contextual := tt})
...
⟨(s.map (λ a, count a s)).fold max 0, le_iff_count.2 $ λ a, begin
  ...
  { refine le_trans _ (mul_le_mul_left _ $ count_pos.2 $ mem_erase_dup.2 h),
    ...
    [simp only [le_max_left] with multiset, simpa [cons_erase h]] },
...
@[simp,multiset] lemma sup_singleton {a : α} : (a::0).sup = a := by simp
...
@[simp,multiset] lemma inf_singleton {a : α} : (a::0).inf = a := by simp
...
multiset.rec_on s {0} (λs _ c, s.bind $ λa, c.map ((::) a))
  (assume a₀ a₁ s pi, by simp [map_bind, bind_bind a₀ a₁, cons_swap] with multiset)
...
multiset.induction_on s (by simp with multiset)
  (assume a s ih, by simp [ih, bind_assoc, map_bind, bind_map, -add_comm] with multiset)
...
multiset.induction_on s (by simp with multiset) (by simp with multiset {contextual := tt})

lemma prod_map_sum [comm_semiring α] {s : multiset (multiset α)} :
  prod (s.map sum) = sum ((sections s).map prod) :=
multiset.induction_on s (by simp with multiset)
  (assume a s ih, by simp [ih, map_bind, sum_map_mul_left, sum_map_mul_right] with multiset)
...
multiset.induction_on m (by simp with multiset) (by simp [mul_comm] with multiset {contextual := tt})
...
by funext; simp [functor.map] with multiset
