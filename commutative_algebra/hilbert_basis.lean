...
#print polynomial.has_scalar
#print finsupp.to_has_scalar'
...
{ zero_ := lattice.bot_le,
  ...
  smul := λ r f Hf,(coeff_zero_iff_deg_le (r • f) n).2 $ λ m Hm,
    begin show (r • f) m = 0, rw finsupp.smul_apply,
      show r • (coeff f m) = 0,
      rw (coeff_zero_iff_deg_le f n).1 Hf m Hm,
      convert smul_zero, end
}
...
lemma leading_term_bdd_deg_ideal {R} [nonzero_comm_ring R] (I : set (polynomial R)) [@is_submodule (polynomial R) _ _ (ring.to_module) I] (n : ℕ) :
submodule R R :=
